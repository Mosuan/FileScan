...
class Reque(object):
    ...
    def query(self, data):
        ...
        for num in range(0, self.num):
            ...
            try:
                if method == "GET":
                    response = requests.request(method, self.url, headers=self.header, verify=False,
                                                timeout=self.timeout)
                elif method == "POST":
                    response = requests.request(method, self.url, data=payload, headers=self.header, verify=False,
                                                timeout=self.timeout)
                break
